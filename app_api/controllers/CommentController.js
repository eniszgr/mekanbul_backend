...
//puanı her seferinde düzenlemek için5
...
const addComment= async function(req,res){
    try {
        await Venue.findById(req.params.venueid).select("comments").exec().then((incomingVenue)=>{createComment(req,res,incomingVenue)});
    } catch (error) {
       ...
       .then(function(venue){
        try {
            ...
            venue.save().then(function(){
                ...
                createResponse(res,"200",{status: comment.author+" isimli yorum silinci"})
            })
        } catch (error) {
            createResponse(res,"400",error);
        }
